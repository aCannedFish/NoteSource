...
> **问题：为什么 server thread 要 detached？**
>
> Thread 有两种状态：
>
...
> ```
>
> 问题：`connfd` 是 main thread 栈上的一个变量，循环下一次 `accept` 可能覆盖它。多个 worker thread 可能读到同一个地址中的新值，导致连接描述符混乱。
>
> 正确做法：为每个连接分配独立 `int`，或用能安全承载整数的 `intptr_t` 做值传递。直接把 `int` 强转为 `void *` 在 64-bit 平台上不可移植。

## Threads 与 Processes 对比
...
> **思考题：Android / HarmonyOS 中 prethreading 可用于什么？**
>
...
>
> > **思考题：为什么 signal handler 中不能使用 `printf`？**
> >
...
> **问题：为什么一个 condition variable 仍然不够？**
>
