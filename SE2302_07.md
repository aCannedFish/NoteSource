...
本部分围绕基于 TCP/IP 的网络程序展开，主要内容包括：

- Client-Server 编程模型。
- 网络、局域网、IP Internet 与 NAT。
- Socket 接口。
- Web Server 与 HTTP 协议。
